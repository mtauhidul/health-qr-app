...
function UploadProgress({
  ...
  const progressBarStyle = {
    ...
    backgroundColor: error ? "#ef4444" : undefined, // Red color for error
  };
    ...
    >
      <div className="bg-card border border-border rounded-lg shadow-lg max-w-sm w-[90%] sm:w-full p-4 sm:p-6 m-4">
          ...
          >
            <svg
              ...
              className={`${error ? "text-destructive" : "text-primary"} ${
                progress < 100 && !error ? "animate-pulse" : ""
              }`}
              style={
                progress < 100 && !error ? { opacity: 0.8 } : { opacity: 1 }
              }
            >
              {error ? (
                // X icon for error
        ...
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>{error ? "Error" : "Upload progress"}</span>
